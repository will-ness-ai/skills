...
Fan out subagents in one batch, one per angle, and cover both sources below and receive an extremely detailed response (written after Skill tool call for writing-for-agents). Each subagent starts blank, so give it a description of the problem and what its trying to find. The subagents read; you judge what comes back, and report.

Examples of things the agents could consider: patterns, libraries, standards, best practices, design/component choices, design patterns, resources and write-up by teams who hit this at scale. Prioritize evidence/proof and reputable sources.
