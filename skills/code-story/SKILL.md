...
4. Check the diagrams with [`check-mermaid.mjs`](check-mermaid.mjs), which runs the real mermaid over every one of them. A diagram mermaid rejects still publishes: it draws a small error box where the picture was, and the page around it looks finished, so a reader finds the hole before you do. Fix what the script names, then run it again until it passes.
