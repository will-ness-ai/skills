...
4. Verify before you publish, not after. The wiring — counter-reset math, elision counter-sets, the data-n↔note link — fails loudly if you get it wrong, so trust it once it renders. What fails silently is anything you generated: reread every quoted string against the real file at the real commit, byte for byte, and if a comment spans lines or carries a parenthetical, keep it whole rather than smoothing it into a clean sentence. Recompute every distance, count, or identifier you wrote from memory (e.g. "three lines above") against the actual lines — either it traces to source or the prose says it's illustrative, never both unlabeled. And for any chapter whose title names a function or mechanism, confirm that mechanism's own lines sit in one of its diff blocks; a title is not evidence. **Done when** every quote is byte-matched, every recalled distance, count, or identifier is traced to source or labeled illustrative, and every mechanism-named chapter title is backed by that mechanism's lines in its own diff block.
5. Publish with the Artifact tool, then hand over the link. Without that tool, write to the scratchpad and `open` the file. `template.html` already is the design, so publish it as it stands.
