...
| `research` | AFK | A fact outside the working directory is blocking a decision. | [research](https://aihero.dev/skills-research), in the ticket's own session; the reading itself runs in a background agent |
...
Every type gets one ticket per session, research included.
...
Underneath, it is mostly other skills wearing wayfinder's scheduling: [grilling](https://aihero.dev/skills-grilling), [domain-modeling](https://aihero.dev/skills-domain-modeling) and [grill-with-docs](https://aihero.dev/skills-grill-with-docs) resolve the default ticket type, [prototype](https://aihero.dev/skills-prototype) resolves the tickets that talking cannot — or [grilling-frontend-prototyping](https://aihero.dev/skills-grilling-frontend-prototyping) where the frontend look is still undecided — and [research](https://aihero.dev/skills-research) takes its own session, where the reading runs in a background agent rather than in your context. [handoff](https://aihero.dev/skills-handoff) is the bridge in and out — into a map from a conversation that outgrew itself, out of one when a side quest appears mid-session. For anything else, [ask-matt](https://aihero.dev/skills-ask-matt) routes over the whole set.
