...
Each question opens with a **zoom out** — a brief on what the agent found and where the question sits, so you're not answering blind — and carries the agent's own recommended answer. Anything the environment can settle, the agent looks up instead of asking you. It won't start enacting the plan until you confirm the shared understanding has been reached.
